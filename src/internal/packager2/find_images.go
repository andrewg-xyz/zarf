...
type FindImagesOptions struct {
	// Path to the helm chart directory within specified git repos
	...
	SkipCosign bool
	// variables for create time package templating
	CreateSetVariables map[string]string
	// variables for deploy time manifest / values-file templating
...
func FindImages(ctx context.Context, packagePath string, opt FindImagesOptions) (map[string][]string, error) {
	l := logger.From(ctx)

	l.Info("using build directory", "path", packagePath)
	...
	return findImages(ctx, pkgLayout.Pkg, opt)
}

// TODO: Refactor to return output string instead of printing inside of function.
func findImages(ctx context.Context, pkg v1alpha1.ZarfPackage, opt FindImagesOptions) (map[string][]string, error) {
	l := logger.From(ctx)
	for _, component := range pkg.Components {
		if len(component.Repos) > 0 && opt.RepoHelmChartPath == "" {
